...
class CommonService(object):
    ...
    def get_instance(self, instance_id, bind_user=True, lock=True):
        query = db.session.query(self._model).filter(self._model.id == instance_id)
        if bind_user:
            query = query.filter(self._model.user_id == g.user.id)
    ...
    def get_data(self, where=None):
        ...
        data = data.filter(getattr(self._model, 'delete_time') == None)
        return data
